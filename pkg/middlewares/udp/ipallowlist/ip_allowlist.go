...
func (al *ipAllowLister) ServeUDP(conn *udp.Conn) {
	...
	if err != nil {
		logger.Error().Err(err).Msgf("Request from %s rejected", addr)
		conn.Close()
		return
	}

	logger.Debug().Msgf("Request from %s accepted", addr)

	al.next.ServeUDP(conn)
}
